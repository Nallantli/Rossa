...
namespace libfs
{
	...
	{
		auto fstr = (std::ofstream *)args[0].getPointer(token).get();
		fstr->close();
		return Symbol();
	}
	...
	{
		auto fstr = (std::ifstream *)args[0].getPointer(token).get();
		fstr->close();
		return Symbol();
	}
	...
	{
		auto path = (boost::filesystem::path *)args[0].getPointer(token).get();
	...
	{
		auto path1 = (boost::filesystem::path *)args[0].getPointer(token).get();
		auto path2 = (boost::filesystem::path *)args[1].getPointer(token).get();
