...
#define _FILE_NOT_FOUND_ "File `{1}` cannot be found"
...
#define _FUNCTION_PARAM_ERROR_ "Parametric values must either be " KEYWORD_FUNCTION " or " KEYWORD_OBJECT "; given type: `{1}`"
...
#define _FAILURE_STR_TO_NUM_ KEYWORD_STRING " `{1}` cannot be converted to " KEYWORD_NUMBER
...
#define _FILE_NOT_FOUND_ "Non si può trovare il file di `{1}`"
...
#define _FUNCTION_PARAM_ERROR_ "Valori parametrici devono essere una Funzione [" KEYWORD_FUNCTION "] o un Oggetto [" KEYWORD_OBJECT "]; il tipo dato: `{1}`"
...
#define _FAILURE_STR_TO_NUM_ "Non si può convertire Stringa [" KEYWORD_STRING "] `{1}` a Numero [" KEYWORD_NUMBER "]"
...
#define _FILE_NOT_FOUND_ "Scāpus quī `{1}` nōminātur nōn invenīrī poterat"
#define _OPTION_NO_STD_ "Optiō --no-std (-ns) dabātur; Generālis Bibliothēca nōn legitur"
#define _UNDECLARED_VARIABLE_ERROR_ "Variābilis quī `{1}` nōminātur nōn in dominiō dēclārātur"
#define _INVALID_OBJECT_TYPE_ "Dēclārātiō Strūctūrae [" KEYWORD_OBJECT "] irrita est"
#define _UNKNOWN_BUILT_CALL_ "Appellātiō prōcessiōnis compīlātae ignōta est: {1}"
#define _UNKNOWN_BINARY_OP_ "Iussus bīnārius ignōtus est: {1}"
#define _UNKNOWN_UNARY_OP_ "Iussus ūnārius ignōtus est: {1}"
...
#define _EXTERN_NOT_DEFINED_ "Prōcessiō extrāria quae `{1}` nōminātur nōn scrībitur"
#define _FUNCTION_PARAM_ERROR_ "Valōrēs parametreī dandī sunt sicut Prōcessiōnem [" KEYWORD_FUNCTION "] aut Strūctūram [" KEYWORD_OBJECT "]; quod datum: `{1}`"
...
#define _FAILURE_STR_TO_NUM_ "Seriēs [" KEYWORD_STRING "] `{1}` in Numerum mūtārī nōn potest"
...
#define _INDEX_OUT_OF_BOUNDS_ "Postulātiō data extrā magnitūdinem est: habet {1}, {2} dabātur"
#define _FAILURE_PARSE_CODE_ "Compilātiō prōcessūs dēfēcit"
#define _EXPECTED_ERROR_ "`{1}` exspectābātur"
...
#define _EXPECTED_AFTER_ "Post `{1}` ēnūntiātiō exspectābātur"
...
#define _FILE_NOT_FOUND_ "ファイルの「{1}」は見つけられなかった"
...
#define _FUNCTION_PARAM_ERROR_ "助変数の値は関数（" KEYWORD_FUNCTION "）かクラス（" KEYWORD_OBJECT "）として上げなくては駄目、上がったのは「{1}」"
...
#define _FAILURE_STR_TO_NUM_ "文字列（" KEYWORD_STRING "）の「{1}」は数にするのは駄目"
